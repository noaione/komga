...
class LibraryContentLifecycle(
  ...
  fun scanSeries(
    ...
    logger.info { "Scan series root folder for series: $series" }
    measureTime {
  ...
  private fun scanAndUpdateFolder(
    ...
    if (scannedSeries.isEmpty() && isLibraryRootPath) {
      ...
      scannedSeries.keys.map { it.url }.let { urls ->
        val tempSeries =
          seriesRepository.findAllNotDeletedByLibraryIdAndUrlNotIn(library.id, urls).filter { s ->
    ...
    val seriesToSortAndRefresh =
      scannedSeries.values.flatten().map { it.url }.let { urls ->
        val books =
          bookRepository.findAllNotDeletedByLibraryIdAndUrlNotIn(library.id, urls).filter { bookUrls ->
    ...
    scannedSeries.forEach { (newSeries, newBooks) ->
      if (!Paths.get(newSeries.url.toURI()).startsWith(rootPath)) {
        logger.info { "Updated series is not from root path: $rootPath" }
        return@forEach
      }
